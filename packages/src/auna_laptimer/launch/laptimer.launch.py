...
def include_launch_description(context: LaunchContext):
    ...
    config_file = PathJoinSubstitution([
        FindPackageShare('auna_laptimer'),
        'config',
        'laptimer.yaml'
    ])

    laptimer_node = Node(
        package='auna_laptimer',
        executable='laptimer',
        name='laptimer_node',
        output='screen',
    ...
    # Add the laptimer node to the launch description content
    launch_description_content.append(laptimer_node)
...
def generate_launch_description():
    ...
    launch_description.add_action(DeclareLaunchArgument(
        'config_file',
        default_value='',
        description='Path to the configuration file for laptimer parameters.'
    ))
